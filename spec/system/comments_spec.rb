...
describe 'コメント機能', type: :system do
  ...
  let(:article_a) { FactoryBot.create(:article, user: user_a, title: 'taitoru') }

  describe 'コメント作成' do
    context '非ログイン時' do
      it 'コメント投稿欄は表示されない' do
        visit article_path(article_a)
        expect(page).to have_current_path(article_path(article_a))
        expect(page).not_to have_field("#{user_a.name}のコメント")
      end
    end
    context 'ログイン時' do
      before do
      ...
      it 'コメント投稿欄は表示される' do
        visit article_path(article_a)
        expect(page).to have_current_path(article_path(article_a))
        expect(page).to have_field("#{user_a.name}のコメント")
      end

      it 'コメント投稿ができる' do
        visit article_path(article_a)
        fill_in "#{user_a.name}のコメント", with: 'テストコメント12345'
        click_button '登録する'
        expect(page).to have_css('.alert-success', text: 'コメントを投稿しました')
        expect(page).to have_content('テストコメント12345')
      end

      it 'コメントが空欄の場合、コメント投稿ができない' do
        visit article_path(article_a)
        click_button '登録する'
        expect(page).to have_selector('#error_explanation', text: 'コメントを入力してください')
      end
    end
  end
